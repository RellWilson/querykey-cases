...
==========================================================================
@fileOverview
Usage guide for the QueryKey Cases MCP adapter (stdio server with JSON-RPC fallback).
@author
Platform Integrations
@maintainer
Platform Integrations
@usage
Run via npm scripts or the published CLI bin `querykey-mcp-adapter`.
@dependencies
Node.js >= 18, optional @modelcontextprotocol/sdk.
@notes
Preview distribution aligned with SDK suite.
@timestamp
...
Via the published adapter (after release):

```bash
npx @querykey/mcp-adapter --help
querykey-mcp-adapter
```
