# coding: utf-8

"""
    QueryKey Cases API

    API including QKey negotiation, version history, API keys (scopes, rotation, soft revoke), usage metering, billing stubs, plan gating, and login lockout mechanics.

    The version of the OpenAPI document: 1.0.0
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from querykey_cases_sdk.models.plan_upgrade_required import PlanUpgradeRequired

class TestPlanUpgradeRequired(unittest.TestCase):
    """PlanUpgradeRequired unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> PlanUpgradeRequired:
        """Test PlanUpgradeRequired
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `PlanUpgradeRequired`
        """
        model = PlanUpgradeRequired()
        if include_optional:
            return PlanUpgradeRequired(
                error = 'plan_upgrade_required',
                required_feature = 'collections.read',
                current_plan = 'free'
            )
        else:
            return PlanUpgradeRequired(
                error = 'plan_upgrade_required',
                required_feature = 'collections.read',
                current_plan = 'free',
        )
        """

    def testPlanUpgradeRequired(self):
        """Test PlanUpgradeRequired"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
