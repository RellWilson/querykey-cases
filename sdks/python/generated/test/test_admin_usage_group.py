# coding: utf-8

"""
    QueryKey Cases API

    API including QKey negotiation, version history, API keys (scopes, rotation, soft revoke), usage metering, billing stubs, plan gating, and login lockout mechanics.

    The version of the OpenAPI document: 1.0.0
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from querykey_cases_sdk.models.admin_usage_group import AdminUsageGroup

class TestAdminUsageGroup(unittest.TestCase):
    """AdminUsageGroup unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> AdminUsageGroup:
        """Test AdminUsageGroup
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `AdminUsageGroup`
        """
        model = AdminUsageGroup()
        if include_optional:
            return AdminUsageGroup(
                endpoint_group = 'cases_read',
                count = 128,
                user_id = d5d8a6cf-3fef-4ea8-8d76-0a038c722c5b,
                total = 512
            )
        else:
            return AdminUsageGroup(
        )
        """

    def testAdminUsageGroup(self):
        """Test AdminUsageGroup"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
